...
Collection.prototype.indexing = function(query, typeIdfDocument, typeTfDocument, typeNormalizationDocument, typeIdfQuery, typeTfQuery, typeNormalizationQuery) {
    ...
    if(typeIdfDocument == 'option2'){
        if(typeNormalizationDocument == 'option2'){
        ...
        else if (typeNormalizationDocument == 'option1'){
            if(typeTfDocument == 'option1'){
                ...
                var max;
                for(var i = 0; i < this.postdata.length; i++){
                    for(var j = 0; j < this.documentWordCounter[i].length; j++){
                        max[i][j] = -1;
                    }
                }
                for(var i = 0; i < this.postdata.length; i++){
                    for(var j = 0; j < this.documentWordCounter[i].length; j++){
                        if (max[i][j] < documentWordCounter[i][j]) {
                            max[i][j] = documentWordCounter[i][j];
                        }
                    }
                }
                for(var i = 0; i < this.postdata.length; i++) {
                    for(var j = 0; j < documentWordCounter[i].length; j++) {
                        id = this.word.indexOf(wordTemp[j]);
                        if(id != -1) {
                            documentweight[i][j] = documentweight[i][j] + math.log(this.postdata.length / this.idf[id], 10) * (0.5 + 0.5 * documentWordCounter[i][j] / max[i][j]);
                        }
                    }
                }
            }
            ...
            if(typeTfDocument == 'option1'){
                ...
                for(var i = 0; i < this.postdata.length; i++) {
                    for(var j = 0; j < documentWordCounter[i].length; j++) {
                        id = this.word.indexOf(wordTemp[j]);
                        if(id != -1) {
                            documentweight[i][j] = documentweight[i][j] + (0.5 + 0.5 * documentWordCounter[i][j] / max[i][j]);
                        }
                    }
                }
            }
    ...
    if(typeIdfQuery == 'option2'){
        if(typeNormalizationQuery == 'option2'){
        ...
        else if (typeNormalizationQuery == 'option1'){
            if(typeTfQuery == 'option1'){
                ...
                for(var i = 0; i < queryWordCounter.length; i++) {
                    if (max[i] < queryWordCounter[i]) {
                        max[i] = queryWordCounter[i];
                    }
                }
                for(var i = 0; i < queryWordCounter.length; i++) {
                    id = this.word.indexOf(wordTemp[i]);    
                    if(id != -1) {
                        queryweight[i] = queryweight[i] + math.log(this.postdata.length / this.idf[id], 10) * (0.5 + 0.5 * queryWordCounter[i] / max[i]);
                    }
                }
            }
        ...
        if(typeNormalizationQuery == 'option2'){
            ...
            if(typeTfQuery == 'option1'){
                ...
                for(var i = 0; i < queryWordCounter.length; i++) {
                    queryweight[i] = queryweight[i] + (0.5 + 0.5 * queryWordCounter[i] / max[i]);
                }
            }
            else if(typeTfQuery == 'option5'){
                for(var i = 0; i < queryWordCounter.length; i++) {
    ...
    for (var i = 0; i < this.postdata.length; i++){
        ...
        for (var j = 0; j < wordTemp.length; j++){
            ...
            // console.log(i+' '+documentweight[i][j]);
        }
        // console.log(i+' '+queryweight[i]);
