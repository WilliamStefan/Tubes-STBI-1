...
Collection.prototype.countRecallPrecision = function() {
    ...
    for(var k = 0; k < this.queryArray.length; k++){
        ...
        // console.log(k + ' '+ wordTemp[k].length);
    }
    for(var k = 0; k < this.queryArray.length; k++){
    ...
    // counting recall
    file.appendFile('recall.txt', 'Query_ID Recall\n');
	for(var i = 0; i < this.queryArray.length; i++){
        ...
        file.appendFile('recall.txt', i+1 + ' ' + this.recall[i]+ '\n');
        // console.log(i + ' '+ this.recall[i]);
    }

    // counting precision
	file.appendFile('precision.txt', 'Query_ID Precision Total_Relevant_Document\n');
    for(var i = 0; i < this.queryArray.length; i++){
        this.precision[i] = this.totalRelevantDocument[i] / this.documentRelevantToQuery[i].length;
        file.appendFile('precision.txt', i+1 + ' ' + this.precision[i] + ' ' + this.documentRelevantToQuery[i].length + '\n');
        // console.log(i + ' '+ this.precision[i]);
    }
    
    file.appendFile('interpolatedprecision.txt', 'Query_ID non-interpolated_Precision \n');
    // counting interpolated
    for(var i = 0; i < this.queryArray.length; i++){
        file.appendFile('interpolatedprecision.txt', i + ' '+ this.interpolatedPrecision[i]+ '\n');
        // console.log(i + ' '+ this.precision[i]);
    }
};
    ...
    if(typeIdfDocument == 'option2'){
        if(typeTfDocument == 'option1'){
            for(var i = 0; i < this.postdata.length; i++) {
                for(var j = 0; j < this.word.length; j++) {
                    if(this.postdata[i][j] != 0){
                        documentweight[i][j] = documentweight[i][id] + math.log(this.postdata.length / this.idf[j], 10);
                    }
                }
            }
        }
