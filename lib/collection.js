...
Collection.prototype.indexing = function(query, typeIdfDocument, typeTfDocument, typeNormalizationDocument, typeIdfQuery, typeTfQuery, typeNormalizationQuery) {
    ...
    var idtemp = 0;
    for(var i = 0; i < documentWordCounter.length; i++){
        for(var j = 0; j < wordTemp.length; j++){
            id = this.postdata.indexOf(wordTemp[j]);
            if(id != -1){
...
Collection.prototype.loadQrels = function(filename) {
    ...
    // for(var i = 0; i < this.relevantDocument.length; i++){
    //     console.log(i + ' '+ this.relevantDocument[i][0] + ' ' + this.relevantDocument[i][1]);
    // }
};
...
Collection.prototype.countRecall = function() {
    ...
    for(var i = 0; i < this.queryArray.length; i++){
        ...
        for(var j = 0; j < this.documentRelevantToQuery.length; j++){
            if(this.relevantDocument[j][0] - 1 == i){
                if(this.relevantDocument[j][1].indexOf(this.documentRelevantToQuery[j]) != -1){
                    this.recall[i]++;
                }
                // console.log(j+ ' ' +this.postdata[j]);
            }
            if(this.relevantDocument[j][0] - 1 > i){
                break;
            }
            console.log(i + ' '+ this.documentRelevantToQuery[j]);
        }
        this.recall[i]/=this.totalRelevantDocument[i];
        console.log(i + ' '+ this.recall[i]);
        break;
    }
};
...
Collection.prototype.countPrecision = function() {
    for(var i = 0; i < this.postdata.length; i++){
        for(var j = 0; j < this.postdata[i].length; j++){
            console.log(i + ' '+ this.postdata[i][j]);
            break;
        }
        // break;
    }
    for(var i = 0; i < this.queryArray.length; i++){
        this.precision[i] = this.totalRelevantDocument[i] / this.postdata.length;
    }

};


Collection.prototype.countInterpolatedPrecision = function() {
    document
};
    ...
    for(var i = 0; i < this.word.length; i++){
        for(var j = 0; j < this.postdata.length; j++){
            if(this.postdata[j][i] != 0)
                wstreamInverted.write(this.word[i] + " " + j + " weight \n");
        }
    }
    wstreamInverted.end();
};
