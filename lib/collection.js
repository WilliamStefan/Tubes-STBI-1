...
Collection.prototype.indexing = function(query, typeIdfDocument, typeTfDocument, typeNormalizationDocument, typeIdfQuery, typeTfQuery, typeNormalizationQuery) {
    var similarity = file.createWriteStream('similarity.txt');
    // typeIdfDocument = 'option2';
    // typeTfDocument = 'option5';
    // typeNormalizationDocument = 'option1';
    // typeIdfQuery= 'option2';
    // typeTfQuery = 'option5';
    // typeNormalizationQuery = 'option1'; 
    file.appendFile('similarity.txt','query: ' + query + '\nDocument_ID Similarity\n');
    ...
    for(var i = 0; i < this.postdata.length; i++){
        if(this.similarities[i] > 0.25){
            file.appendFile('similarity.txt',(i+1)+' '+this.similarities[i]+'\n');
        }
    }
...
Collection.prototype.loadQrels = function(filename) {
    ...
    for(var i =0; i < this.queryArray.length; i++){
        this.totalRelevantDocument[i] = 0;
    }
...
Collection.prototype.countRecallPrecision = function() {
    ...
    var precisionrecall = file.createWriteStream('precisionrecall.txt');
    
    file.appendFile('precisionrecall.txt', 'Query_ID Recall Precision non-interpolated_Precision Total_Relevant_Document\n');
    ...
    for(var i = 0; i < this.queryArray.length; i++){
        ...
        file.appendFile('precisionrecall.txt', i+1 + '\t' + this.recall[i] + '\t'+ this.precision[i] + '\t' + this.interpolatedPrecision[i] + '\t' + this.documentRelevantToQuery[i].length + '\n');
    }
    precisionrecall.end();
};
    ...
    for(var i = 0; i < this.word.length; i++){
        for(var j = 0; j < this.postdata.length; j++){
            ...
            if(this.postdata[j][i] != 0){
                ...
                countLala++;
                console.log(i + ' ' + documentweight[j][i]);
            }
        }
    }
...
Collection.prototype.indexing2 = function(query, typeIdfDocument, typeTfDocument, typeNormalizationDocument, typeIdfQuery, typeTfQuery, typeNormalizationQuery) {
    // typeIdfDocument = 'option2';
    // typeTfDocument = 'option5';
    // typeNormalizationDocument = 'option1';
    // typeIdfQuery= 'option2';
    // typeTfQuery = 'option5';
    // typeNormalizationQuery = 'option1'; 
    var interactive = file.createWriteStream('interactive.txt');
    file.appendFile('interactive.txt','query: ' + query + '\nDocument_ID Similarity\n');
    for(var i = 0; i < this.similarities.length; i++){
        this.similarities[i] = 0;
    }
    var temp = this.stemText(this.removeStopWord(query));
    ...
    if(typeNormalizationDocument == 'option2' && typeNormalizationQuery == 'option2'){
        for (var i = 0; i < this.postdata.length; i++){
            ...
            // file.appendFile('interactive.txt',(i+1)+' '+this.similarities[i]+'\n');
        }
    }
    else if(typeNormalizationDocument == 'option2' && typeNormalizationQuery == 'option1'){
        for (var i = 0; i < this.postdata.length; i++){
    ...
    for(var i = 0; i < this.similarities.length; i++){
        tempSim[i]=[];
        tempSim[i][0] = this.similarities[i];
        tempSim[i][1] = i;
    }
    tempSim.sort().reverse();
    for(var i = 0; i < this.similarities.length; i++){
        file.appendFile('interactive.txt',tempSim[i][1]+' '+tempSim[i][0]+'\n');
    }
    var prec = 0, rec = 0;

    var tempRelevanDoc = 0;
    for(var i = 0; i < wordTemp.length; i++){
        for(var j = 0; j < documentWordCounter[i].length; j++){
            if(documentWordCounter[i][j] != 0){
                tempRelevanDoc++;
            }
        }
    }
    prec = tempRelevanDoc/documentWordCounter.length;
    rec = documentWordCounter.length/this.postdata.length;
    file.appendFile('interactive.txt', 'Precision: '+prec+'\n');
    file.appendFile('interactive.txt', 'Recall: '+rec+'\n');
    interactive.end();
}
